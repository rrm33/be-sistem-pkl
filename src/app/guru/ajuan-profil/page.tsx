...
import { UserCheck, Check, X, Eye, CheckCircle2, Clock, XCircle, Filter } from 'lucide-react';
import { toast } from 'sonner';
import Swal from 'sweetalert2';
import Cookies from 'js-cookie';

export default function AjuanProfilGuruPage() {
    ...
    try {
      ...
      if (token) {
        try {
          ...
          guruId = payload.guruId || payload.sub;
        } catch (e) {}
      }

      const url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/pengajuan-edit?status=${selectedStatus}${guruId ? `&guruId=${guruId}` : ''}`;
  ...
  const handleApproveSingle = (id: number, namaSiswa: string) => {
    const guruName = Cookies.get('userName') || 'Guru Pembimbing';
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/pengajuan-edit/${id}/approve`, {
            ...
            body: JSON.stringify({ approvedBy: `${guruName} (Guru Pembimbing)` })
          });
          if (res.ok) {
      ...
      if (result.isConfirmed && result.value) {
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/pengajuan-edit/${id}/reject`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ alasanPenolakan: result.value, approvedBy: `${guruName} (Guru Pembimbing)` })
          });
          if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-smk-blue uppercase tracking-wider mb-1">
            <UserCheck size={16} /> Verifikasi Bimbingan Siswa
          </div>
          <h1 className="text-2xl font-black text-slate-900 leading-tight">Ajuan Edit Profil Siswa Bimbingan</h1>
          <p className="text-sm text-slate-500 font-medium">
            Verifikasi dan setujui perubahan data profil yang diajukan oleh siswa bimbingan PKL Anda.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
        {[
          { label: 'Menunggu Persetujuan', value: 'PENDING', icon: Clock },
          { label: 'Disetujui', value: 'DISETUJUI', icon: CheckCircle2 },
          { label: 'Ditolak', value: 'DITOLAK', icon: XCircle },
          { label: 'Semua Riwayat', value: 'ALL', icon: Filter }
        ].map(tab => {
          const Icon = tab.icon;
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">Belum ada siswa bimbingan yang mengajukan perubahan data.</p>
          </div>
        ) : (
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm">
              ...
              {selectedAjuan.status !== 'PENDING' && (
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs font-semibold text-slate-600">
                  ...
                  <div>
                    Diproses Oleh: <span className="font-bold text-slate-900">{selectedAjuan.approvedBy || 'Guru Pembimbing'}</span>
            ...
            {selectedAjuan.status === 'PENDING' && (
              <div className="p-6 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-3">
                <button
                  ...
                  className="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-sm cursor-pointer"
                >
                  Tolak Ajuan
                </button>
                <button
                  onClick={() => handleApproveSingle(selectedAjuan.id, selectedAjuan.siswa.namaLengkap)}
                  className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm cursor-pointer"
                >
                  Setujui Ajuan
                </button>
              </div>
